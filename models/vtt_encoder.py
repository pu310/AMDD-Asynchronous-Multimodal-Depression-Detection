# -*- coding: utf-8 -*-
# 你可以把这个新模块放在一个单独的文件里，或者直接放在vtt_encoder.py的顶部
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x):
        b, t, c = x.shape
        assert t <= self.pos_embedding.num_embeddings, f"Sequence length {t} > max sequence length {self.pos_embedding.num_embeddings}"
...
class VTTEncoder(nn.Module):
    """
    VTTEncoder: A hierarchical attention network for sequence classification.
    ...
    def __init__(self, seq_len=1356, in_chans=136, num_classes=1, dims=[64, 128, 256, 512],
                 ...
                 tt_depth=2, tt_heads=6, tt_mlp_dim_ratio=2,
        ...
        if self.use_temporal_transformer:
           self.temporal_transformer = TemporalTransformer(
                dim=dims[-1], # 输入维度是VTTEncoder最后一层的维度
                depth=tt_depth,
                heads=tt_heads,
                mlp_dim=dims[-1] * tt_mlp_dim_ratio,
                max_seq_len=256 # 【重要】这个值要大于等于VTTEncoder输出的最大序列长度
            ).to(device)
    ...
    def forward(self, x):
        """
        Forward pass through the VTTEncoder.
        ...
        '''
        if self.use_temporal_transformer:
            # 1. 维度换位以适配Transformer: [B, C, T] -> [B, T, C]
            x_seq = x.permute(0, 2, 1)
            
            # 2. 送入TemporalTransformer进行深度时序建模
            x_seq = self.temporal_transformer(x_seq)
            
            # 3. 维度换回以便后续处理 (如果需要): [B, T, C] -> [B, C, T]
        ...
        # print(f"classifier x_vtt_cls: {x_vtt_cls.shape}")
